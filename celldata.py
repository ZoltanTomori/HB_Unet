...
class CellDataset(Dataset):
    ...
    def __init__(
        ...
        pct=.8, data_type='train', in_size=572, out_size=388,
        ...
        """ self.root_dir = os.getcwd() if not root_dir else root_dir
        path = os.path.join(self.root_dir, 'data\\video1')
        self.train_path = os.path.join(path, 'images')
        self.mask_path = os.path.join(path, 'masks')
        self.test_path = os.path.join(path, 'TestImages') """
        ...
        for i in range(numImages):
            pathImg = os.path.join(self.train_path, str(i)+'.png')
            pathMask = os.path.join(self.mask_path, str(i)+'.png')
            if (os.path.exists(pathImg)==False) or (os.path.exists(pathMask)==False):
                break
            img = io.imread(pathImg, as_gray=True)
            img = resize(img, (512, 512), anti_aliasing=True)
            myImages.append(img)    
            mask = io.imread(pathMask, as_gray=True)
            mask = resize(mask, (512, 512), anti_aliasing=True)
            myMasks.append(mask) 

        self.n_classes = 2
        self.images = np.array(myImages, dtype=np.float64)
        self.masks = np.array(myMasks, dtype=np.float64)"""
